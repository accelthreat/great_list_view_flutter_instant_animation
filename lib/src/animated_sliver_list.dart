...
class AnimatedListController {
  ...
  AnimatedRenderSliverList get adaptor => _adaptor;
  ...
  void notifyInsertedRange(final int from, final int count) {
    ...
    if (_adaptor.isReordering) {
      _adaptor._stopReordering(true);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        notifyInsertedRange(from, count);
      });
      return;
    }
  ...
  void notifyRemovedRange(final int from, final int count,
      final NullableIndexedWidgetBuilder removeItemBuilder) {
    _checkAdaptor();

    if (_adaptor.isReordering) {
      _adaptor._stopReordering(true);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        notifyRemovedRange(from, count, removeItemBuilder);
      });
      return;
    }
  ...
  void notifyChangedRange(final int from, final int count,
      final NullableIndexedWidgetBuilder changeItemBuilder) {
    _checkAdaptor();

    if (_adaptor.isReordering) {
      _adaptor._stopReordering(true);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        notifyChangedRange(from, count, changeItemBuilder);
      });
      return;
    }
  ...
  /// Returns the [State] of the item at the specified position.
  /// If the state cannot be retrieved, `null` is returned.
  T stateOf<T extends State<StatefulWidget>>(int index) {
    _checkAdaptor();
    return _adaptor.childManager.stateOf(index);
  }
}
...
class DefaultAnimatedListAnimationBuilder
  ...
  Widget buildReordering(BuildContext context, Widget widget, int index) {
    return Opacity(
        child: Material(elevation: 16.0, child: widget), opacity: 0.8);
  }
}
...
class AnimatedRenderSliverList extends RenderSliverList
  ...
  AnimatedListInterval _insertNewInterval(
      final int from,
      final int removeCount,
      final int insertCount,
      final NullableIndexedWidgetBuilder removeItemBuilder,
      [final AnimatedListIntervalEventCallback onDisposed]) {
    if (removeCount == 0 && insertCount == 0) return null;
  ...
  double _adjustScrollOffset() {
    ...
    if (offsetCorrection != 0.0) {
      ...
      parentData.layoutOffset += offsetCorrection;
    }
  ...
  double get currentReorderingChildOffset {
    return isReordering
        ? (_reorderStartOffset +
            ...
            .clamp(0.0, geometry.maxPaintExtent - _reorderChildSize) as double
        : 0.0;
  }
...
class AnimatedSliverChildBuilderDelegate extends SliverChildDelegate {
  ...
  final bool Function(int index, int dropIndex, dynamic slot) onReorderComplete;
  ...
  Widget build(BuildContext context, int index) {
    ...
    final buildType = childManager?.buildType ?? AnimatedListBuildType.UNKNOWN;
...
NullableIndexedWidgetBuilder _shiftItemBuilder(
    final NullableIndexedWidgetBuilder builder, final int leading) {
